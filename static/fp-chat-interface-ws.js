...
class ModernChatInterface {
  ...
  async sendViaWebSocket(query) {
    ...
    // Create conversation on server if we don't have one
    if (!this.currentConversationId || !this.wsConversationId) {
      try {
  ...
  handleWebSocketMessage(event) {
    try {
      ...
      // Handle raw streaming data (like EventSource)
      if (data.message_type) {
        // Reuse the same handler that EventSource uses
        this.handleStreamingData(data);
      }
    } catch (error) {
  ...
  handleStreamingData(data) {
    ...
    let messageContent = this.currentStreamingMessage.content || '';
    let allResults = this.currentStreamingMessage.allResults || [];
    ...
    // Handle the existing message types
    const result = this.handleStreamingMessageWithResult(data, textDiv, messageContent, allResults);
    if (result) {
  ...
  handleAIResponse(data) {
    ...
    // Clear loading dots on first real content
    if (!this.currentStreamingMessage.started && (data.content || data.message_type)) {
      textDiv.innerHTML = '';
      this.currentStreamingMessage.started = true;
    }
    
    // Always store debug messages
    this.debugMessages.push({
      type: data.message_type || 'ai_response',
      data: data,
      timestamp: new Date().toISOString()
    });
    
    // The AI response from WebSocket will have the content in a structured format
    // Parse the content if it's a string containing JSON
    if (data.content && typeof data.content === 'string') {
      try {
        // Try to parse as JSON for structured responses
        const parsedContent = JSON.parse(data.content);
        if (parsedContent.message_type) {
          // It's a structured message, handle it
          const context = {
            messageContent: messageContent,
            allResults: allResults
          };
          this.handleStreamingMessage(parsedContent, textDiv, context);
      ...
      // Content is already an object with message_type
      const context = {
        messageContent: messageContent,
        allResults: allResults
      };
      this.handleStreamingMessage(data.content, textDiv, context);
      messageContent = context.messageContent;
      allResults = context.allResults;
    }
    
    // Update current streaming message state
    this.currentStreamingMessage.content = messageContent;
    this.currentStreamingMessage.allResults = allResults;
    
    // Check if streaming is complete
    if (data.is_complete || data.type === 'complete') {
      this.endStreaming();
    }
  }
