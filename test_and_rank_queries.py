#!/usr/bin/env python3
"""
Test queries against agentfinder.azurewebsites.net/who and rank by quality
"""
...
def test_query(query: str) -> Tuple[str, Dict]:
...
def rank_result_quality(query: str, result) -> Dict:
    ...
    if isinstance(content, list):
        ...
        # - At least 1 result: +10 points
        # - 3+ results: +20 points
        # - 5+ results: +30 points
        # - Average score > 0.5: +20 points
        # - Average score > 0.7: +30 points
        # - Average score > 0.8: +40 points
...
def main():
    ...
    print("Testing queries against agentfinder.azurewebsites.net/who...")
    print("=" * 80)

    results = []
    for i, query in enumerate(queries, 1):
        print(f"[{i}/{len(queries)}] Testing: {query[:60]}...")
        query_text, result = test_query(query)
        ...
        if quality['error']:
            ...
            print(f"    ✓ Results: {quality['num_results']}, "
                  f"Avg Score: {quality['avg_result_score']:.3f}, "
    ...
    if queries_with_results > 0:
        avg_results = sum(r['num_results'] for r in results) / queries_with_results
        print(f"Average results per successful query: {avg_results:.2f}")

    print("\n" + "=" * 80)
    print("TOP 10 QUERIES BY QUALITY")
    print("=" * 80)
    ...
    print("\n" + "=" * 80)
    print("BOTTOM 10 QUERIES (from tested set)")
    print("=" * 80)
